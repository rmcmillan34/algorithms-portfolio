...
\begin{Thinking}
    ...
    We can make the above better by implementing a $hash table$ allowing for \(O(1)\) lookup of seen integers.
\end{Thinking}

\Algorithm
\begin{CodeBlock}[style=pseudo,caption={Pseudocode}]{12}
  let hm be a hash_map
...
\Complexity
The algorithm runs in \(O(n)\) time, where \(n\) is the number of digits in the input array. It has an auxiliary space complxity of \(O(1)\) 
