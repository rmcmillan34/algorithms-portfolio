...
\begin{itemize}[nosep]
    ...
    \item Local adjacency decisions: next/previous comparison, runs, peaks/valleys
\end{itemize}
...
\begin{CodeBlock}[style=pseudo,caption={Arrays: Core Operations (pseudocode)}]{18}
access(A, i):
    return A[i] // O(1)
...
% State the key invariants maintained (e.g., head/tail consistency, ordering, shape).
...
\begin{itemize}[nosep]
  ...
  \item Prefix sums / scan: $O(n)$ time, $O(n)$ extra space (or $O(1)$ in place if possible)
